...
pub fn run(json_out: bool) -> Result<()> {
    ...
    if json_out {
        ...
        println!("  CLI detected:  {}", detected_cli.unwrap_or_else(|| "none".into()));
        println!("  API keys set:  {}", if api_keys.is_empty() { "(none)".into() } else { api_keys.join(", ") });
    }
    Ok(())
}

fn detect_cli_context() -> Option<String> {
    if std::env::var("CLAUDECODE").is_ok() { return Some("claude-code".into()); }
    if std::env::var("CURSOR_TRACE_ID").is_ok() { return Some("cursor".into()); }
    if std::env::var("GEMINI_CLI").is_ok() { return Some("gemini-cli".into()); }
    if std::env::var("CODEX_SESSION").is_ok() { return Some("openai-codex".into()); }
    if std::env::var("FACTORYAI").is_ok() { return Some("factory-ai".into()); }
    None
}
