...
fn keypoints_box(mut doc: Docx, body: &str) -> Docx {
    ...
    // Title paragraph: carries the flavor sentinel + `keep_next` so
    // it never page-splits away from the first bullet (reference
    // emits `<w:keepNext/>` on the title — see Round-V brief).
    ...
    let mut out = Cursor::new(Vec::<u8>::new());
    {
        ...
        for name in &order {
            if name == "word/document.xml" {
                ...
                // Round-V Zone-E (visual parity, 2026-06-03): inject
