-- =============================================================================
--  Migration 0012: allow the 'freshness' checkpoint in audit_verdicts
-- =============================================================================
--  ADR-0047 (R9): the domain-freshness gate records its verdict. SQLite cannot
--  ALTER a CHECK constraint, so rebuild the table with the expanded allow-list.
...
CREATE TABLE audit_verdicts_new (
    ...
    checkpoint      TEXT NOT NULL CHECK (checkpoint IN (
        ...
        -- freshness gate (migration 0012, ADR-0047)
        'freshness'
    )),
